...
insert_item(int item, long int id)
{
    ...
    int check = BUFFER_SIZE + 1;
    while (check > BUFFER_SIZE) {
        sem_getvalue(&sem, &check);
    }
...
remove_item(int *item, long int id)
{
    ...
    int check; // = 0;
    if (sem_getvalue(&sem, &check) < 1) sem_wait(&sem); 
    // if (buffer.value[buffer.next_out % BUFFER_SIZE] < 1) sem_wait(&sem);    // minskar automagiskt?
